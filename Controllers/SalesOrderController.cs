...
namespace SalesOrder.Controllers
{
    public class SalesOrderController(IOrderRepository orderRepository) : Controller
    {
        ...
        public async Task<IActionResult> Index(string keyword, DateTime? orderDate, DateTime? orderDateTo, int pageNumber = 1)
        {
            int pageSize = 15; 
            var query = _orderRepository.GetOrdersQueryable(keyword, orderDate, orderDateTo);
            ...
            ViewBag.OrderDateTo = orderDateTo?.ToString("yyyy-MM-dd");
        ...
        {
            decimal totalAmount = await _orderRepository.GetTotalOrderAmountAsync(orderId);
        ...
        public async Task<IActionResult> Edit(int id, int? pageNumber)
        {
            ...
            int pageSize = 10;
            var query = _orderRepository.GetOrdersQueryable(null, null,null);
