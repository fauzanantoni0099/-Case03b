...
public class OrderRepository : IOrderRepository
{
    ...
    public async Task<List<OrderDto>> GetOrdersAsync(string keyword, DateTime? orderDate, DateTime? orderDateTo)
    {
        ...
        if (orderDate.HasValue && orderDateTo.HasValue)
        {
            query = query.Where(o => o.OrderDate >= orderDate.Value && o.OrderDate <= orderDateTo.Value);
        }
        else if (orderDate.HasValue)
        {
            query = query.Where(o => o.OrderDate >= orderDate.Value);
        }
        else if (orderDateTo.HasValue)
        {
    ...
    public IQueryable<OrderDto> GetOrdersQueryable(string keyword, DateTime? orderDate, DateTime? orderDateTo)
    {
        var query = _context.SoOrders
            .Include(o => o.Customer)
            .AsQueryable();
    ...
    {
        decimal totalAmount = await _context.SoItems
            .Where(oi => oi.SoOrderId == orderId)
            .SumAsync(oi => (decimal)(oi.Quantity * oi.Price));
        return totalAmount;
    }
}